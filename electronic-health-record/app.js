		...
		try {
			...
			*/
			...
			app.post('/api/create-patient-mysql', async (req, res) => {
				...
				const insertPatientQuery = `INSERT INTO patients (PatientID, FirstName, LastName, DateOfBirth, Gender, ContactInfo) 
											VALUES (?, ?, ?, ?, ?, ?)`;
			  
				const insertAccountQuery = `INSERT INTO accounts (Username, Password, UserType, UserID) 
											VALUES (?, ?, ?, ?)`;
				  ...
				  if (error) {
					...
					db.query(insertAccountQuery, [username, password, 'patient', patientID], (accountError) => {
					  if (accountError) {
						...
						res.status(200).json({ message: 'Patient and account created successfully' });
					  }
					});
				  }
				});
			...
			app.post('/api/create-doctor-mysql', async (req, res) => {
				...
				// Insert data into MySQL database
				const insertDoctorQuery = `INSERT INTO doctors (DoctorID, FirstName, LastName, Specialization, ContactInfo, HospitalID) 
										  VALUES (?, ?, ?, ?, ?, ?)`;
				  ...
				  if (error) {
					...
					db.query(insertAccountQuery, [username, password, 'doctor', doctorID], (accountError) => {
					  if (accountError) {
						...
						console.log('Account created successfully');
						res.status(200).json({ message: 'Doctor and account created successfully' });
					  }
					});
				  }
				});
            ...
            */
			...
			try {
				...
				async (error, patientResults) => {
					...
					const insertLogQuery = `
						INSERT INTO logChanges (Timestamp, Username, UserType, UserID)
						VALUES (?, ?, ?, ?);
					`;

					db.query(insertLogQuery, [timestamp, username, 'patient', patientID], (logError) => {
						if (logError) {
			...
			try {
				...
				async (error, doctorResults) => {
					...
					db.query(insertLogQuery, [timestamp, username, 'doctor', doctorID], (logError) => {
						if (logError) {
						console.error('Failed to insert into logChanges table:', logError);
			...
			try {
				...
				async (error, recordResults) => {
					...
					db.query(insertLogQuery, [timestamp, username, 'medicalRecord', recordID], (logError) => {
						if (logError) {
						console.error('Failed to insert into logChanges table:', logError);
